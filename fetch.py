    ...
    def generate(self, num_candidates=1, offset=1, pattern_string="0,0", use_index=True):
        ...
        for narrative_line in self.narrative:
            ...
            for output_type in output_pattern:
                if output_type == 0:
                    similar_chunks = self.get_similar_chunks(source_chunk, num_candidates, remove_stopwords=True, offset=offset, use_index=use_index)
                    if num_candidates > 1:
...
if __name__ == "__main__":
    ...
    mosaic.generate(num_candidates=num_candidates, offset=offset, pattern_string=pattern_string, use_index=use_index)

    # mosaic.print_play()

    mosaic.save_play(sys.argv[6])
